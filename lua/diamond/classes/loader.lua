...
do
	...
	local _base_0 = {
		...
		downloadAssets = function(self, dirs)
			local __laux_type = (istable(dirs) and dirs.__type and dirs:__type()) or type(dirs)
			assert(__laux_type == "table", "Expected parameter `dirs` to be type `table` instead of `" .. __laux_type .. "`")
			for i, v in pairs(dirs) do
				local __lauxi5 = self.package
				assert(__lauxi5 ~= nil, "cannot destructure nil value")
				local root = __lauxi5.root
				local path = i .. "/" .. root
		...
		finish = function(self)
			local __lauxi6 = Diamond.Core.Loader.logTheme
			assert(__lauxi6 ~= nil, "cannot destructure nil value")
			local text, success = __lauxi6.text, __lauxi6.success
			local __lauxi7 = self.package
			assert(__lauxi7 ~= nil, "cannot destructure nil value")
			local assets = __lauxi7.assets
