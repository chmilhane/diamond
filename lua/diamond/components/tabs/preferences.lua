...
function PANEL:Init()
    ...
    for i, v in pairs(theme:list()) do
        ...
        Diamond:on("themeChanged", function(oldTheme, newTheme)
            if (not label or not IsValid(label)) then return end
            label:SetTextColor(theme:get("text"))
        end)
        ...
        for x, color in pairs(v) do
            if (x == "black" or x == "white") then continue end
            ...
            function btn:Paint(w, h)
                ...
                if prefMode == x or prefColor == x then
                    draw.SimpleText(utf8.char(0xf00c), Diamond:Font(22, {
                    font = "Font Awesome 5 Free Solid" }), w / 2, h / 2, theme:get((prefMode == "light" and x == "light") and "black" or "white"), 1, 1)
                end
            end
